const { triggerEvent, useOneResponse } = require('./eventHandler')
...
const createDataHandler = (triggerEvent, useOneResponse) => (
	...
	responses.forEach(commandResponse => {
		...
		const isEvent =
			breakCommand(response.heos.command).command_group === 'event'
		if (isEvent)
			...
			useOneResponse(connection, response.heos.command, {
				message: parseMessage(response.heos.message)
					? parseMessage(response.heos.message).message
					: undefined,
				payload: response.payload
...
const dataEventHandler = createDataHandler(triggerEvent, useOneResponse)
